...
useSeoMeta({
  ...
  description: 'Turn a logo into favicon.ico, apple-touch-icon, Android icons, a web manifest and the markup to paste in — as one zip. Rendered in your browser, never uploaded. Free and open source.',
...
const howToSteps = [
  ...
  { title: 'Download the pack', body: 'One zip: favicon.ico, the PNGs, site.webmanifest and a head.html with the markup. Unzip it into the root of your site.' },
...
useToolJsonLd(meta, {
  ...
  featureList: [
    ...
    'Apple touch icon and Android home screen icons',
    'Web manifest and ready-to-paste markup',
...
const faq = [
  ...
  { q: 'Why only six files? Other generators produce thirty.', a: 'Because most of those thirty are for platforms that no longer exist. Windows tile images, a dozen iOS sizes from the era before iOS scaled icons itself, and the various Android densities are all legacy. Modern browsers take the ICO or the 32px PNG, iOS takes the 180px apple-touch-icon, and Android reads 192 and 512 from the manifest. Anything else is bytes nobody requests.' },
  ...
  { q: 'Should my logo have padding?', a: 'Usually a little. A logo designed to be seen at 200 pixels often has fine detail and tight margins that turn to mush at 16. Ten to fifteen percent of padding, and simplifying the mark itself if you can, does more for legibility than any amount of resampling.' },
  { q: 'Why does my icon look black on my iPhone?', a: 'Because the source is transparent. iOS does not preserve transparency in home-screen icons — it composites them onto black. Tick the background option and pick a colour that suits your logo, and the apple-touch-icon comes out right.' },
  { q: 'Can I use an SVG?', a: 'Yes, as the source. Your browser rasterises it at each size, which generally gives a cleaner result than starting from a small PNG. The output is always PNG and ICO, because that is what the icon slots actually accept.' },
  { q: 'Is my logo uploaded?', a: 'No. Every size is drawn by a canvas in your browser and packaged into a zip there too. Nothing is sent anywhere, which for an unreleased logo is a meaningful difference from the usual arrangement.' },
  { q: 'Where do the files go?', a: 'The root of your site, next to your index page, so they resolve at /favicon.ico and /site.webmanifest as the markup expects. If you put them in a subfolder, adjust the paths in the snippet to match.' },
]
</script>
...
<template>
  <div class="container-page py-10">
    <ToolPageHeader
      ...
      description="Turn a logo into every icon a site actually needs — ICO, Apple touch icon, Android icons, manifest and markup, in one zip. Nothing is uploaded."
    />
